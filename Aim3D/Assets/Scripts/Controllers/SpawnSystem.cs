...
public class SpawnSystem : MonoBehaviour
{
    ...
    void spawnCheck()
    {
        if (_enemiesToSpawn > 0 && Time.time > _nextSpawnTime != null)//checks if there are enemies left to spawn this wave and if it is time to spawn a enemy, if it executes it ajusts the amount of enemies that need to spawn, resets the timer and spawns a enemy
        {
            _enemiesToSpawn--;
